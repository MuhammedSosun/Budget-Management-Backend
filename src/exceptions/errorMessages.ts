...
export const ErrorMessages: Record<ErrorCode, string> = {
  ...
  [ErrorCode.RATE_LIMIT_EXCEEDED]:
    "You are performing actions too fast, please slow down.",
  ...
  [ErrorCode.DEFAULT_WORKSPACE_CREATE_FAILED]:
    "Failed to create default workspace.",
  [ErrorCode.WORKSPACE_OWNER_MEMBER_CREATE_FAILED]:
    "Failed to create workspace owner member.",
  [ErrorCode.WORKSPACE_MEMBER_NOT_FOUND]: "Workspace member not found.",
  [ErrorCode.WORKSPACE_MEMBER_NOT_IN_WORKSPACE]:
    "The workspace member does not belong to the specified workspace.",
  [ErrorCode.WORKSPACE_MEMBER_ROLE_UPDATE_FAILED]:
    "Failed to update member role.",
  [ErrorCode.WORKSPACE_MEMBER_USER_NOT_FOUND]: "Member user not found.",
  [ErrorCode.OWNER_ROLE_CANNOT_BE_CHANGED]:
    "The role of the OWNER user cannot be changed.",
  ...
  [ErrorCode.IDEMPOTENCY_REQUEST_ALREADY_PROCESSED]:
    "This request has already been processed. Please use a new idempotency key.",
  [ErrorCode.RATE_LIMIT_EMAIL_RESEND]:
    "Too many verification emails sent. Please wait 10 minutes.",
  [ErrorCode.RATE_LIMIT_TOO_FAST]:
    "You are sending requests too fast. Please slow down.",
  [ErrorCode.RATE_LIMIT_REFRESH]: "Refresh token rate limit exceeded.",
  [ErrorCode.DEFAULT_WORKSPACE_CANNOT_BE_DELETED]:
  ...
  [ErrorCode.DEFAULT_WORKSPACE_CANNOT_BE_LEFT]:
    "Default workspace cannot be left.",
  [ErrorCode.WORKSPACE_OWNER_CANNOT_LEAVE]: "Owner cannot leave the workspace.",

  [ErrorCode.BUDGET_LIMIT_ALREADY_EXISTS]:
    "Budget limit for this category and period already exists.",
  ...
  [ErrorCode.PASSWORD_RESET_IF_EMAIL_EXIST]:
    "If an account with this email address exists, a password reset link has been sent.",
  [ErrorCode.FAILED_TO_SEND_PASSWORD_RESET_LINK]:
